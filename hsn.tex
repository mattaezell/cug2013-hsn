...
software uses the MMRs control network performance and identify problems, but
they are also accessible to users (most readily through CrayPat).

\subsection{Network Links}

Each Gemini chip supports two nodes, so the nodes share a topology coordinate.
Between Gemini chips, there are many ``lanes'' that work in concert to transfer
data.  Three lanes comprise a ``link'' that maps to a single network tile.
Four links connect in each of the positive and negative Y dimension.  For X and
Z, there are eight links in each direction.
...
network is able to run in auto-degrade mode.  The lane with an abnormal number
of errors is deactivated and the Gemini balances the traffic over the other two
lanes within the channel.  For a configurable period of time, the Gemini will
attempt to re-initialize the failed lane to restore full bandwidth.  An example
of a degraded lane is shown in Figure \ref{fig:lanedegrade}. 
...
action to allow communication to continue.  Network traffic is quiesced and new
routing tables (excluding the failed links) are computed and asserted to each
Gemini chip.  Then traffic is again allowed to flow using the updated routes.
